...
int main(int argc, char ** argv) {
  ...
  printf("startup: set socket opt CAN_RAW_FD_FRAMES\n");
...
;
    if (setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (const char *)&recv_timeout,
      ...
      int e = errno;
      printf("setsockopt() SRCVTIMEO  failed. %d %s \n", e, strerror(e));
      exit(1);
    }
  } else {
  ...
  if (nbytes < 0) {
    ...
    printf("recvd %x %d 0x", recvframe.can_id, recvframe.len);
    for (int i=0; i<recvframe.len; i++) {
      printf("%02x",recvframe.data[i]);
      }
    printf("\n");
  }
  } // while
